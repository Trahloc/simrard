...
pub fn scorer_builder_impl(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let generated = quote! {
        impl #impl_generics ::simrard_lib_utility_ai::scorers::ScorerBuilder for #component_name #ty_generics #where_clause {
            #build_method
            #label_method
        }
    };
...
fn get_label(input: &DeriveInput) -> Option<LitStr> {
    ...
    for option in attrs {
        if let Meta::NameValue(meta_name_value) = &option.meta {
            if meta_name_value.path.is_ident("scorer_label") {
